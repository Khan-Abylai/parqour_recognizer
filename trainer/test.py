...
def validation(model, criterion, evaluation_loader, converter, opt, device):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        labels = [label for label in labels]  # Оставляем текст как есть
        ...
        if 'CTC' in opt.Prediction:
            preds = model(image, text_for_pred)  # Прогон модели
            forward_time = time.time() - start_time

            # Подготовка к CTC Loss
            preds_size = torch.IntTensor([preds.size(1)] * batch_size).to(device)
            cost = criterion(
                preds.log_softmax(2).permute(1, 0, 2).to(device),  # Убедимся, что это на `device`
                text_for_loss,
                preds_size,
                length_for_loss
            )
